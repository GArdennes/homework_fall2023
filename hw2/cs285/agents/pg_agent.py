...
class PGAgent(nn.Module):
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            # values = ptu.to_numpy(self.critic.forward(ptu.from_numpy(obs)).detach())   
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    # HINT: remember that advantages at time t are a sum of immediate rewards and the discounted
                    if i == batch_size - 1:
                        next_value = last_value
                    else:
                        next_value = values[i + 1]
                    delta = rewards[i] + self.gamma * next_value * (1 - terminals[i]) - values[i]
                    advantage = delta + self.gamma * self.gae_lambda * (1 - terminals[i]) * advantage
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discounted_returns = sum([a*b for a,b in zip(discounts, rewards)])
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        n_steps = len(rewards)
        discounted_rewards_to_go = np.zeros(n_steps)
        # Compute the return G_t with the rewards-to-go formulation,
        # which is more efficient than the standard formulation
        for t in range(n_steps)[::-1]:
            disc_return_t = (discounted_rewards_to_go[0] if len(discounted_rewards_to_go)>0 else 0)
            discounted_rewards_to_go[t] = ( self.gamma*disc_return_t + rewards[t])
